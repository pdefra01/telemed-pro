...
- **Mandatory Mercado Pago step**: every sign-up ends in a Mercado Pago step, whatever the plan or payment option. The affiliation is not completed if the payment link cannot be created: the form shows an error and a "Reintentar" that repeats only the Mercado Pago call for the same request (never a second request). If Mercado Pago secrets are missing the endpoints return 503 and sign-ups cannot complete.
  ...
  - **Checkout Pro first-period payment**: Familiar with cash, transfer, Rapipago and link; later periods are invoiced. The `payment` webhook (external reference `adhesion:<id>:checkout`) records the payment (including the amount actually paid) on the adhesion request and posts it into the affiliate ledger through the idempotent service-role RPC `post_adhesion_checkout_payment`: first invoice of the current month, charge and payment movements, invoice marked paid only when payments cover it, `plan_status` active, and a coverage window opened only if the affiliate has none (an existing one is never extended). Whichever happens second posts it: the webhook when the request is already approved, or approve-adhesion when the payment arrived first. RPC errors are retried by the deferred reconciliation (`ledger_post_failed`); business refusals (e.g. affiliate without a plan) go to admin review (`ledger_not_posted`, `needs_admin`). An overpayment leaves a credit; an existing invoice total is never changed.
