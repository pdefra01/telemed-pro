...
- [x] T7 Docs (MANUAL_DE_USUARIO.md, PRD.md, docs/prd-mercadopago-debito-automatico.md; delegated writer, reviewed by the orchestrator via diff stat)
...
- T5 (delegated writer): RED (16/38 failing) then GREEN. Full suite 544 pass / 5 pre-existing fails, verified by the orchestrator. Plans list shows kind/option/offered/price/months/size/commission; in-use plans lock price and terms; "Crear nueva version" prefills a draft (not offered). Not tested: modal/Agreements rendering (pure helper tests only). Resolved later: publishing a new version now auto-withdraws the old one (migration 20260920040000, user-approved). Open: commission is locked with the price when a plan is in use.
...
- Migrations 20260920020000, 030000, 040000 applied to remote with `supabase db push --linked` (user OK). 030000 first failed: both kept patients shared a phone; user gave Alejandro Mayer a new phone (3876116344), then it applied. Remote catalog verified: 5 offered plans with the agreed prices/commissions, default = Plan Familiar. A legacy plan "Plan individual" (no kind, not offered) still exists in remote and is untouched.
