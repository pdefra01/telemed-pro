...
src/pages/AdhesionForm.tsx:1437: step-6 copy now reads "Te enviaremos un correo con un enlace para crear tu contraseña y acceder a la App de MEDINEX." No DNI-as-password claim found anywhere in the file (grepped for DNI/contraseña/email/correo -- the only other DNI/correo references are the unrelated OTP-email-verification and form-field labels, confirmed by manual read). src/pages/__tests__/AdhesionForm.test.tsx updated with an assertion for the new copy and absence of the old claim (7/7 passing in that file).
